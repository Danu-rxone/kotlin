...
fun soal02_defaultArg(nama: String, bahasa: String /* TODO */): String {
    return "$nama belajar $bahasa"
}

// 3. Named Arguments. Fungsi bawaan: func(a: Int, b: Int, c: Int).
// Kembalikan PANGGILAN FUNGSI di bawah tersebut dengan mendahulukan memasukkan C = 3, lalu B = 2, A = 1 ke input.
fun mockFunc(a: Int, b: Int, c: Int) = "$a$b$c"
fun soal03_namedArg(): String {
    // TODO: return mockFunc(...)
    return ""
}

// 4. Single-Expression Function. Hapus kurung kurawal, dan jadikan fungsi satu baris dengan `=`.
fun soal04_singleExpr(teks: String): Int = 0 // TODO: Ganti 0 dgn mengukur panjang string (length)

// 5. Vararg. Fungsi bisa menerima data list/array bebas menggunakan keyword `vararg`.
// Jumlahkan SEMUA angka dari `numbers` tersebut, dan kembalikan angkanya. (bisa pakai perulangan atau method bawaan return numbers.sum())
...
// Ubah signature kembalian fungsi ini dari `String` DARI FUNGSINYA (di kanan kurung) menjadi `String?`! LALU return null murni
fun soal06_nullableType(): String /*TODO*/ {
    return "" // Ganti jadi null
}

// 7. Pengecekan Null Klasik (If-Else). Jika masukan `nama` null, return "Tanpa Nama". Jika tidak return namanya.
fun soal07_nullIf(nama: String?): String {
    // TODO
    return ""
}

// 8. Safe Call Operator `?.`. Kembalikan panjang dari password input (jika String). Jika inputan null, KEMBALIKAN NULL (Int?).
...
// 11. Extension Functions. Kita bisa membuat sub-fungsi tempelan untuk Kelas yg sudah ada (seperti String).
// Fungsi `String.hurufTerakhir()` akan dibuat. 
// TUGAS ANDA: Panggil ekstensi `.hurufTerakhir()` pada argumen teks tersebut dan return karakter hasilnya!
...
// Jika data adalah null, return -1. Jika data bertipe Int, kalikan 10. Jika String, kembalikan length-nya.
fun soal12_smartCastLagi(data: Any?): Int {
    // TODO
    return 0
}

// 13. Unit Returning. Fungsi default jika void/tidak mengembalikan apapun maka tipenya otomatis `Unit`.
// Fungsi ini haruslah ber-return type eksplisit `Unit` di parameter method dan memiliki isian bebas, misalnya `println()`.
...
// Cukup panggil `soal15_higherOrder` di dalam TODO block, dan PASS LAMBDA ke dalamnya `soal15_higherOrder { it + 5 }`.
